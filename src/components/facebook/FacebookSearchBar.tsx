...
interface FacebookSearchBarProps {
  username: string;
...
export const FacebookSearchBar = ({ 
  username, 
  onUsernameChange,
  isLoading = false,
  onSearch
}: FacebookSearchBarProps) => {
  ...
  return (
    <div className="flex flex-col space-y-3 w-full">
      ...
      <Button
        ...
        className="w-full h-11 bg-[#1877F2] hover:bg-[#1877F2]/90 rounded-xl shadow-sm 
                   hover:shadow-md transition-all duration-300 ease-spring text-white
                   disabled:bg-[#1877F2]/50 disabled:hover:bg-[#1877F2]/50"
        variant="secondary"
      >
        {isLoading ? (
          <>
