  ...
  return (
    <Card 
      className={`p-6 space-y-6 relative w-[280px] transition-all duration-300 
        ${isPopular ? 'instagram-gradient animate-pulse shadow-xl scale-105 bg-card/50 backdrop-blur-sm' : ''}`}
    >
      {isPopular && (
      ...
      <div className="space-y-3">
        <h2 className={`text-lg font-semibold ${isPopular ? 'text-white' : ''}`}>{name}</h2>
        <p className="text-[11px] text-muted-foreground leading-relaxed">{description}</p>
        <div className={`text-xl font-bold ${isPopular ? 'text-white' : ''}`}>
          {isAnnual ? (
            <>
